...
I adapted the code from this Oracle blog post to remove outliers and impute missing values. 
...
The base model is naive. To make it more robust, first, I assess the correlation between the series and its time lags (autocorrelation). In both cases, ACF (correlation a time series and its lags) test shows only a couple of lags have statistically significant estimated correlation coefficients. As expected, PACF (correlation between a time series and its lags unexplained by previous lags) test shows overall even small coefficients. 
...
visualize_its_adjusted_od_st <- function(input){
  ...
  model <- glm(n ~ intervention + date + group + harmonic(date, 2, length(input$date)), family = quasipoisson, data = input) # harmonic term is added for accounting for seasonality
