...
ggsave("/home/jae/ITS-Text-Classification/output/cleaned_data_plot.png", height = 10)
```
...
visualize_base <- function(input){
  ...
  model <- lm(count_ts ~ intervention + date + intervention*date + group, data = input)
...
ggarrange(pacf_plot(df_domestic) + ggtitle("Domestic Political Interests (PACF)"), 
          pacf_plot(df_nondomestic) + ggtitle("International Political Interests (PACF)"),
           nrow = 2, ncol =1)
```
...
correct_ac <- function(a, b, input){
  ...
  model <- gls(count_ts ~ intervention + date + intervention*date + group, 
               data = input,
...
pq_domestic <- bind_rows(
correct_ac(1, 1, df_domestic),
correct_ac(1, 2, df_domestic),
correct_ac(2, 1, df_domestic),
correct_ac(2, 2, df_domestic)
)

pq_nondomestic <- bind_rows(
correct_ac(1, 1, df_nondomestic),
correct_ac(1, 2, df_nondomestic),
correct_ac(2, 1, df_nondomestic),
correct_ac(2, 2, df_nondomestic)
)
...
visualize_adj <- function(input, p.pam, q.pam){
  ...
  model <- gls(count_ts ~ intervention + intervention*date + date + group, 
               data = input,
...
ggarrange(visualize_adj(df_domestic, 2, 2) + ggtitle("9/11 and Minority Domestic Political Interests"), 
          visualize_adj(df_nondomestic, 1, 1) + ggtitle("9/11 and Minority International Political Interests"),
           nrow = 2, ncol =1)
...
model_output <- gls(count_ts ~ intervention + intervention*date + date + group, 
               data = df_domestic,
               correlation = corARMA(p= 2, q = 2, form = ~ date | group),
               na.action = na.omit) 

stargazer(model_output)
