...
```{r}
corpus <- quanteda::corpus(corpus, text_field = "clean_text")
toks <- quanteda::tokens(corpus)

mod1 <- conText(formula = terror ~ group, 
          data = toks, 
...
mod2 <- conText(formula = terror ~ group, 
          data = toks, 
          pre_trained = local_glove,
          transform = TRUE, 
          transform_matrix = local_transform, 
...
mod3 <- conText(formula = terror ~ group, 
          data = toks, 
          pre_trained = local_glove,
          transform = TRUE, 
          transform_matrix = local_transform, 
...
sums <- data.frame(estimate = 
             c(mod1@normed_cofficients$Normed_Estimate,
             mod2@normed_cofficients$Normed_Estimate,
             mod3@normed_cofficients$Normed_Estimate),
           std.error = 
             c(mod1@normed_cofficients$Std.Error, mod2@normed_cofficients$Std.Error, mod3@normed_cofficients$Std.Error),
           group = c("Indian American", "Filipino American", "Asian American"))
