...
subtitle: "Prepared for the KDI School of Public Policy and Development"
...
# Data availability and empirical social science research 
...
* **Relatively difficult** to investigate how minority groups **experience** and **internalize such bias**
...
* How can we do a 1) **large-scale**, 2) **well-designed**, and 3) **cost-effective** observational study on marginalized populations?
...
```{r include = FALSE}
df <- read_csv(make_here("/home/jae/ITS-Text-Classification/processed_data/df.csv"))[,-1]
```

```{r}
df %>%
  select(intervention, date, group, text) %>%
  head(3)
```
...
![](https://github.com/jaeyk/ITS-Text-Classification/raw/master/output/duration_effect.png)
